...
@echo off
 ...
 :run
  ...
  for /F "tokens=3 delims=: " %%H in ('sc query "Service Labtice 0.1" ^| findstr "        STATE"') do (
    if /I "%%H" NEQ "RUNNING" (
      REM Put your code you want to execute here
      REM For example, the following line
	  net start "Service Labtice 0.1"
    ) else (
      taskkill -f -im Labtice.exe
      taskkill -f -im App.WindowsService.exe
